...
import { SkeletonBlockResult, SkeletonListResult } from './SkeletonLoaders';
    ...
    <section class="mx-auto w-full p-4 bg-gray-100 dark:bg-gray-800">
      <Show when={true}>
        <div class="mx-auto sm:max-w-3xl block items-center justify-center h-full text-gray-600 dark:text-gray-400 p-3">
          ...
          <Show when={!store.state.results.isLoading} fallback={<SkeletonListResult />}>
            <For
              each={store.state.results.blockList}
              fallback={
                <div class="basis-1/3 flex py-2 justify-start">No Blocks</div>
              }
            >
              {(item) => (
